<template>
    <section class="card" >
        <custom-data-table :columns="columns" :url="'/api/v1/answers-types'" :id="'AnswersTypesDataTable'" :load="loadTables" :xprops="xprops"></custom-data-table>
    </section>
</template>
...
export default {
    data() {
        return {
            xprops: {
                module: 'AnswersTypesIndex',
                route: 'answers_types',
                permission_prefix: 'answers_type_',
                eventbus: new Vue()
            },
            columns: [
    ...
    computed: {
        ...mapGetters('AnswersTypesIndex', ['data', 'total', 'loading', 'relationships']),
    },
    methods: {
        ...mapActions('AnswersTypesIndex', ['fetchData', 'setQuery', 'resetState']),
        showTable() {
