<template>
    <section class="card" >
        <custom-data-table :columns="columns" :url="'/api/v1/communication-skills'" :id="'CommunicationSkillsDataTable'" :load="loadTables" :xprops="xprops"></custom-data-table>
    </section>
</template>
...
export default {
    data() {
        return {
            xprops: {
                module: 'CommunicationSkillsIndex',
                route: 'communication_skills',
                permission_prefix: 'communication_skill_',
                eventbus: new Vue()
            },
            columns: [
    ...
    computed: {
        ...mapGetters('CommunicationSkillsIndex', ['data', 'total', 'loading', 'relationships']),
    },
    methods: {
        ...mapActions('CommunicationSkillsIndex', ['fetchData', 'setQuery', 'resetState']),
        showTable() {
