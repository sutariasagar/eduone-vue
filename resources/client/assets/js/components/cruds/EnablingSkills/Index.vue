<template>
    <section class="card" >
        <custom-data-table :columns="columns" :url="'/api/v1/enabling-skills'" :id="'EnablingSkillsDataTable'" :load="loadTables" :xprops="xprops"></custom-data-table>
    </section>
</template>
...
export default {
    data() {
        return {
            xprops: {
                module: 'EnablingSkillsIndex',
                route: 'enabling_skills',
                permission_prefix: 'enabling_skill_',
                eventbus: new Vue()
            },
            columns: [
    ...
    computed: {
        ...mapGetters('EnablingSkillsIndex', ['data', 'total', 'loading', 'relationships']),
    },
    methods: {
        ...mapActions('EnablingSkillsIndex', ['fetchData', 'setQuery', 'resetState']),
        showTable() {
