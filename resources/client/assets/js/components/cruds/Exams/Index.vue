    ...
    data() {
        return {
            ...
            columns: [
                ...
                {data: 'question_pool.label', label: 'Question Pool', name: 'question_pool', sortable: true,group: 'advanced', orderable: "false",searchable: true, },
                ...
                {data:'updated_by', name: 'updated_by.name', searchable: true, visible:false, label: "Updated By", group: 'advanced'},
                {
                    ...
                    "render": function (data, type, row, meta) {
                        ...
                        if (ability.can(context.xprops.permission_prefix + 'view')) {
                            ...
                            anchor.attr('link','/'+context.xprops.route + '/' + id + '/students');
                            anchor.attr('route',context.xprops.route);
                            anchor.attr('params',id);
                            anchor.attr('action','showstudents');
                            anchor.append('<i class="icmn-user"></i>');
                            anchor.addClass('btn btn-icon btn-primary btn-rounded mr-2 mb-2 custom-link')
                            div.append(anchor)
                        }
                        if (ability.can(context.xprops.permission_prefix + 'view')) {
                            let anchor = $('<a></a>');
