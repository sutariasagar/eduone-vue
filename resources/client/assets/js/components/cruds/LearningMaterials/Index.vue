<template>
    <section class="card" >
        <custom-data-table :columns="columns" :url="'/api/v1/learning-materials'" :id="'LearningMaterialsDataTable'" :load="loadTables" :xprops="xprops"></custom-data-table>
    </section>
</template>
...
export default {
    data() {
        return {
            xprops: {
                module: 'LearningMaterialsIndex',
                route: 'learning_materials',
                permission_prefix: 'learning_material_',
                eventbus: new Vue()
            },
            columns: [
    ...
    computed: {
        ...mapGetters('LearningMaterialsIndex', ['data', 'total', 'loading', 'relationships']),
    },
    methods: {
        ...mapActions('LearningMaterialsIndex', ['fetchData', 'setQuery', 'resetState']),
        showTable() {
