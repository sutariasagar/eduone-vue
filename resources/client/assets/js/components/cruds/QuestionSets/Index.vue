<template>
    <section class="card" >
        <custom-data-table :columns="columns" :url="'/api/v1/question-sets'" :id="'QuestionSetsDataTable'" :load="loadTables" :xprops="xprops"></custom-data-table>
    </section>
</template>
...
export default {
    data() {
        return {
            xprops: {
                module: 'QuestionSetsIndex',
                route: 'question_sets',
                permission_prefix: 'question_set_',
                eventbus: new Vue()
            },
            columns: [
    ...
    computed: {
        ...mapGetters('QuestionSetsIndex', ['data', 'total', 'loading', 'relationships']),
    },
    methods: {
        ...mapActions('QuestionSetsIndex', ['fetchData', 'setQuery', 'resetState']),
        showTable() {
