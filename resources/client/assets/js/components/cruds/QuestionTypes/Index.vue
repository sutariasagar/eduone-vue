<template>
    <section class="card" >
        <custom-data-table :columns="columns" :url="'/api/v1/question-types'" :id="'QuestionTypesDataTable'" :load="loadTables" :xprops="xprops"></custom-data-table>
    </section>
</template>
...
export default {
    data() {
        return {
            xprops: {
                module: 'QuestionTypesIndex',
                route: 'question_types',
                permission_prefix: 'question_type_',
                eventbus: new Vue()
            },
            columns: [
                {data:'id', name: 'id', label: "Id", group: 'basic'},
    ...
    computed: {
        ...mapGetters('QuestionTypesIndex', ['data', 'total', 'loading', 'relationships']),
    },
    methods: {
        ...mapActions('QuestionTypesIndex', ['fetchData', 'setQuery', 'resetState']),
        showTable() {
