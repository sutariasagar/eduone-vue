<template>
    <draggable tag="ul" :list="item.question_details.ordered_text.suggestions" class="list-group">
        <li class="list-group-item" v-for="(suggestion, index) in item.question_details.ordered_text.suggestions" :key="index">
            <template>
    ...
    export default {
        ...
        methods: {
            ...
            addItemInList: function () {
                let list = JSON.parse(JSON.stringify(this.item.question_details.ordered_text.suggestions));
                list.push({text: "", chosen: false});
                console.log(list);
                //this.$set(this, "list", list);
                this.setQuestionDetailsByKey({key: 'ordered_text', value: {suggestions: JSON.parse(JSON.stringify(list))}})
            },
