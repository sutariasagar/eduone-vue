...
<script>
    ...
    export default {
        ...
        watch: {
            list: function () {
                this.setQuestionDetailsByKey({key: 'fib_drag_drop', value: {suggestions: JSON.parse(JSON.stringify(this.list))}})
            }
        },
        mounted() {
            if(this.$route.name == "questions_banks.edit") {
                this.$set(this, "list", JSON.parse(JSON.stringify(this.item.question_details.fib_drag_drop.suggestions)))
            }
        },
        methods: {
