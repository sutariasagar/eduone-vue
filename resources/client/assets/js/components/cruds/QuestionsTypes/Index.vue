<template>
    <section class="card" >
        <custom-data-table :columns="columns" :url="'/api/v1/questions-types'" :id="'QuestionsTypesDataTable'" :load="loadTables" :xprops="xprops"></custom-data-table>
    </section>
</template>
...
export default {
    data() {
        return {
            xprops: {
                module: 'QuestionsTypesIndex',
                route: 'questions_types',
                permission_prefix: 'questions_type_',
                eventbus: new Vue()
            },
            columns: [
    ...
    computed: {
        ...mapGetters('QuestionsTypesIndex', ['data', 'total', 'loading', 'relationships']),
    },
    methods: {
        ...mapActions('QuestionsTypesIndex', ['fetchData', 'setQuery', 'resetState']),
        showTable() {
