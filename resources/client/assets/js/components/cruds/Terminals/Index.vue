<template>
    <section class="card" >
        <custom-data-table :columns="columns" :url="'/api/v1/terminals'" :id="'TerminalsDataTable'" :load="loadTables" :xprops="xprops"></custom-data-table>
    </section>
</template>
...
<script>
    ...
    export default {
        data() {
            return {
                xprops: {
                    module: 'TerminalsIndex',
                    route: 'terminals',
                    permission_prefix: 'terminal_',
                    eventbus: new Vue()
                },
                columns: [
                    ...
                    {data: 'user', name: 'user.name', searchable: true, label: " Name", group: 'basic', visible:true},
        ...
        computed: {
            ...mapGetters('TerminalsIndex', ['data', 'total', 'loading', 'relationships']),
        },
        methods: {
            ...mapActions('TerminalsIndex', ['fetchData', 'setQuery', 'resetState']),
            showTable() {
