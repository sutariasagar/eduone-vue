...
const actions = {
    storeData({ commit, state, dispatch }) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (_.isEmpty(state.item.skill)) {
                ...
                for (let index in state.item.skill) {
                    params.set('skill['+index+']', state.item.skill[index].id)
                }
            }
            

            axios.post('/api/v1/enabling-skills', params)
                .then(response => {
                    commit('resetState')
                    resolve()
                })
    ...
    updateData({ commit, state, dispatch }) {
        ...
        return new Promise((resolve, reject) => {
            ...
            axios.post('/api/v1/enabling-skills/' + state.item.id, params)
                .then(response => {
    ...
    fetchData({ commit, dispatch }, id) {
        axios.get('/api/v1/enabling-skills/' + id)
            .then(response => {
                commit('setItem', response.data.data)
            })
    ...
    setSkill({ commit,dispatch }, value) {
        dispatch('Alert/removeError', 'skill', { root: true });
